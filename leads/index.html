...
.hdr-count{background:var(--bg);border:1px solid var(--border);color:var(--text);font-size:13px;font-weight:600;padding:6px 12px;font-family:inherit;display:inline-flex;align-items:center;height:32px;}
.hdr-btn{background:var(--bg);border:1px solid var(--border);color:var(--muted);padding:0;width:32px;height:32px;font-size:13px;cursor:pointer;font-family:inherit;transition:background .12s;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;}
.hdr-btn:active{background:var(--border);}
.avatar{width:32px;height:32px;border-radius:4px;background:var(--slate);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:600;color:#fff;cursor:pointer;overflow:hidden;}
...
/* SEARCH */
.search-wrap{background:var(--white);padding:10px 14px;border-bottom:1px solid var(--border);}
...
.lead-row{padding:13px 14px;display:flex;align-items:center;gap:12px;cursor:pointer;}
...
.lead-badge{display:inline-flex;align-items:center;gap:3px;font-size:10px;font-weight:600;color:var(--green);background:rgba(26,122,69,0.08);border:1px solid rgba(26,122,69,0.18);border-radius:3px;padding:2px 6px;margin-top:4px;}
...
.cal-day{height:46px;display:flex;align-items:center;justify-content:center;background:var(--white);border:1px solid var(--border);font-size:16px;font-weight:600;color:var(--text);font-family:inherit;cursor:pointer;padding:0;}
...
.time-slot{height:46px;display:flex;align-items:center;justify-content:center;background:var(--white);border:1px solid var(--border);font-size:15px;font-weight:600;color:var(--text);font-family:inherit;cursor:pointer;padding:0;direction:ltr;}
...
<div id="note-modal">
  <div class="modal-sheet">
    ...
    <div class="mtitle">סיכום שיחה</div>
    <div class="msub" id="modal-name"></div>
    ...
    <button class="btn btn-skip" onclick="closeModal()">סגור בלי לשמור</button>
  </div>
</div>
...
<div id="meeting-modal" style="display:none;position:fixed;inset:0;background:rgba(26,26,40,0.45);z-index:340;align-items:flex-end;backdrop-filter:blur(4px);">
  <div class="modal-sheet" style="padding:18px 16px 22px;padding-bottom:calc(22px + env(safe-area-inset-bottom));max-height:94dvh;overflow-y:auto;">
    <div class="mhandle"></div>
    <div class="mtitle" style="font-size:19px;font-weight:700;margin-bottom:4px">קביעת פגישה</div>
    <div class="msub" id="meeting-name" style="font-size:14px;color:var(--slate);margin-bottom:14px;font-weight:600"></div>

    <!-- Section: VISUAL CALENDAR -->
    <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px">
        <button type="button" onclick="navMonth(-1)" class="picker-btn" style="width:46px;min-height:46px;font-size:22px">‹</button>
        <div id="cal-title" style="flex:1;text-align:center;font-size:18px;font-weight:700;color:var(--slate)">—</div>
        <button type="button" onclick="navMonth(1)" class="picker-btn" style="width:46px;min-height:46px;font-size:22px">›</button>
      </div>
      <!-- Day-of-week header — Sunday first -->
      <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:4px;margin-top:4px">
        <div class="cal-dow">א'</div>
        <div class="cal-dow">ב'</div>
        <div class="cal-dow">ג'</div>
        <div class="cal-dow">ד'</div>
        <div class="cal-dow">ה'</div>
        <div class="cal-dow">ו'</div>
        <div class="cal-dow">ש'</div>
      </div>
      <!-- Calendar days grid — populated by renderCalendar() -->
      <div id="cal-grid" style="display:grid;grid-template-columns:repeat(7,1fr);gap:4px"></div>
    </div>

    <!-- Section: TIME SLOTS — half-hours from 08:00 to 20:00 -->
    <div style="margin-bottom:14px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
        <label style="font-size:14px;font-weight:700;color:var(--slate)">שעת התחלה</label>
        <div style="font-size:18px;font-weight:700;color:var(--slate);background:var(--bg);border:1px solid var(--border);padding:6px 12px" id="time-current">--:--</div>
      </div>
      <div id="time-slots" style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px"></div>
      <!-- Manual entry row — for times outside the preset range or odd minutes -->
      <div style="display:flex;align-items:center;gap:8px;margin-top:10px;background:var(--bg);border:1px solid var(--border);padding:10px;direction:ltr">
        <span style="font-size:12px;color:var(--muted);font-weight:600;writing-mode:horizontal-tb">או הזן ידנית:</span>
        <input type="number" id="manual-hour" min="0" max="23" placeholder="HH" style="width:70px;padding:10px;background:var(--white);border:1px solid var(--border);font-size:18px;font-weight:700;font-family:inherit;color:var(--text);text-align:center" oninput="onManualTimeChange()"/>
        <span style="font-size:24px;font-weight:700;color:var(--slate)">:</span>
        <input type="number" id="manual-minute" min="0" max="59" placeholder="MM" style="width:70px;padding:10px;background:var(--white);border:1px solid var(--border);font-size:18px;font-weight:700;font-family:inherit;color:var(--text);text-align:center" oninput="onManualTimeChange()"/>
      </div>
    </div>

    <!-- end time preview -->
    <div style="background:var(--bg);padding:12px 14px;border:1px solid var(--border);font-size:14px;display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
      <span style="color:var(--muted);font-weight:600">שעת סיום (+1 שעה)</span>
      <span id="meeting-end-preview" style="font-weight:700;color:var(--slate);font-size:20px">—</span>
    </div>
    ...
    <div style="display:flex;gap:10px">
      <button class="btn btn-save" style="flex:1.4;padding:18px 12px;font-size:17px;font-weight:700" onclick="saveMeeting()">שמור פגישה</button>
      <button class="btn btn-skip" style="flex:1;padding:18px 12px;font-size:16px;font-weight:600" onclick="closeMeetingDialog()">ביטול</button>
    </div>
  </div>
</div>
</div>
...
async function loadLeads(){
  ...
  try{
    const res=await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/A:W?key=${API_KEY}&_=${Date.now()}`,{cache:'no-store'});
    ...
    const all=(data.values||[]).slice(1)
      .map((row,i)=>({
        ...
        callers:((row[22]||'')+'').trim()                 // column W (0-based 22)
      }))
...
function renderLeadCard(l,kind){
  ...
  let actionBtn;
  if(kind==='removed'){
    actionBtn=`<button class="restore-btn" onclick="event.stopPropagation();restoreLead(${l.index})" title="שחזר">${ICON.restore}</button>`;
  } else if(kind==='continue'){
    actionBtn=`<button class="call-btn ${done?'done':''}" onclick="event.stopPropagation();callLead(${l.index},'${ej(l.name)}','${ej(l.phone)}')">${done?ICON.check:ICON.phone}</button>`;
  } else {
    actionBtn=`<button class="call-btn ${done?'done':''}" onclick="event.stopPropagation();callLead(${l.index},'${ej(l.name)}','${ej(l.phone)}')">${done?ICON.check:ICON.phone}</button>`;
  }
  ...
  return `<div class="lead-row" onclick="toggleExpand(${l.index})">
    <div class="lead-info">
...
function renderRemoved(leads){
  ...
  leads.forEach(l=>{
    // Removed tab keeps the original behavior — no swipe, just a restore button (✓).
    const d=document.createElement('div');
    d.className='lead-card'+(l.note?' has-note':'')+(expandedSet.has(l.index)?' expanded':'');
    d.id='removed-card-'+l.index;
    d.innerHTML=renderLeadCard(l,'removed');
    list.appendChild(d);
  });
}
...
async function setLeadStatus(idx,status,toastMsg){
  ...
  try{
    const res=await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/V${idx}?valueInputOption=RAW`,
...
/* =========================
   CALLER MARK — writes to column W (append user tag)
   ========================= */
async function markCalled(idx){
  ...
  if(list.map(s=>s.toLowerCase()).includes(tag))return; // already marked
  list.push(tag);
  const newVal=list.join(', ');
  if(lead)lead.callers=newVal;
  filterLeads();
  try{
    await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/W${idx}?valueInputOption=RAW`,
      {method:'PUT',headers:{'Authorization':'Bearer '+accessToken,'Content-Type':'application/json'},
...
function filterLeads(){
  ...
  const active=allLeads.filter(l=>(!l.status||l.status===STATUS_ACTIVE)&&match(l));
  const cont=allLeads.filter(l=>l.status===STATUS_CONTINUE&&match(l));
  const rem=allLeads.filter(l=>l.status===STATUS_REMOVED&&match(l));
  renderLeads(active);
  renderContinue(cont);
  renderRemoved(rem);
  // update header counter for the current tab
  const counts={active:active.length,continue:cont.length,removed:rem.length};
  document.getElementById('lead-count').textContent=counts[currentTab];
}
...
function pickTime(h,m){
  ...
  // Clear manual inputs so it's clear which one is active
  document.getElementById('manual-hour').value='';
  document.getElementById('manual-minute').value='';
  renderTimeSlots();
  syncHiddenInputs();
}
...
async function saveMeeting(){
  ...
  try{
    ...
    // Fire the Make webhook so the calendar event + WhatsApp run immediately,
    // without waiting for the polling cycle. We don't await the response — the
    // sheet write already succeeded, so Make can pick up the work in parallel.
    fireMeetingWebhook({
      rowNum,
      name:lead.name||'',
      phone:lead.phone||'',
...
async function saveNote(){
  ...
  try{
    const res=await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/C${row}?valueInputOption=RAW`,
      {method:'PUT',headers:{'Authorization':'Bearer '+accessToken,'Content-Type':'application/json'},body:JSON.stringify({values:[[combined]]})});
    const data=await res.json();
    if(data.error)throw new Error(data.error.message);
    expandedSet.add(row);showToast('נשמר','ok');filterLeads();
  }catch(e){showToast(e.message,'err');}
  finally{document.getElementById('saving-overlay').classList.remove('show');}
}
