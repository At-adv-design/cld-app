...
.call-dots{display:flex;align-items:center;gap:4px;padding:6px 14px 0;direction:ltr;justify-content:flex-start;}
...
async function loadLeads(){
  ...
  try{
    const res=await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/A:AB?key=${API_KEY}&_=${Date.now()}`,{cache:'no-store'});
    const data=await res.json();
    if(data.error)throw new Error(data.error.message);
    const all=(data.values||[]).slice(1)
      .map((row,i)=>{
        const meta=((row[27]||'')+'').trim();
        ...
        return {
          ...
          // Column AB (0-based 27) — full audit log: status changes, calls,
...
function renderLeadCard(l,kind){
  ...
  return `<div class="lead-row-wrap">
    <div class="call-dots">${renderCallDots(l.callCount||0)}</div>
...
async function setLeadStatus(idx,status,toastMsg){
  ...
  try{
    ...
    // Network/auth failure → save to queue so we don't lose the action.
    // The queue is replayed on reconnect / next visibility event.
...
async function appendMetaLog(idx, event){
  ...
  const url=`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/AB${idx}?valueInputOption=RAW`;
  const opts={method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({values:[[combined]]})};
  try{ await apiFetch(url,opts); }
  catch(e){ enqueueWrite(url,opts,'יומן ליד'); }
}
...
async function markCalled(idx){
  ...
  try{ await apiFetch(url,opts); }
  catch(e){ enqueueWrite(url,opts,'סימון מתקשר'); }
...
async function saveNote(){
  ...
  try{
    ...
    showToast('שמור — יסונכרן כשהחיבור יחזור','ok');
    expandedSet.add(row);filterLeads();
  }
...
async function saveAndMove(target){
  ...
  if(note){
    ...
    const opts={method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({values:[[combined]]})};
    try{ await apiFetch(url,opts); }
    catch(e){ enqueueWrite(url,opts,'סיכום שיחה'); }
    finally{document.getElementById('saving-overlay').classList.remove('show');}
  }
...
(function attachDragToClose(){
  ...
  ['note-modal','meeting-modal','log-view-modal','log-edit-modal','reminders-modal','confirm-dialog']
    .forEach(attach);
})();
